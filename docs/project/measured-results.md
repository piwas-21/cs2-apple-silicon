...
`docs/project/development-plan.md` has 34 tasks. Roughly half are **software** (they can be finished on any machine and
...
| T-002 licence and rules | **SHIPPED** | `LICENSE` (GPL-3.0, verbatim), `docs/legal-and-vac.md` §Distribution model — one box ticked, no open question |
...
| T-008 install CS2, close the depot gap | **TOOLED** | `docs/install.md` step 6, `cs2kit doctor` depot checks |
...
| T-010 `[GATE]` offline playable | **HUMAN** | `docs/install.md` step 10 (three maps, twice each, 30 min) |
| T-011 benchmark protocol + baseline | **SHIPPED** as tooling | `cs2kit bench run/import`, `docs/benchmarking.md` |
...
| T-015 input latency | **HUMAN** | `docs/benchmarking.md`; needs a 240 fps camera and 20 trials |
| T-016 audio and microphone | **HUMAN** | `docs/troubleshooting.md` audio entries; needs a real teammate |
| T-017 thermals, 2-hour soak | **TOOLED** | `bench run --label soak`, `powermetrics` availability recorded |
| T-018 account safety | **SHIPPED** as policy | `docs/project/validation-log.md` checklist |
| T-019 casual + community servers | **HUMAN** | `docs/project/validation-log.md` networking checklist |
| T-020 `[GATE]` VAC-protected competitive | **HUMAN** | `docs/project/validation-log.md` match log + GO/CONDITIONAL/NO-GO block. **Unresolved: this is the project's actual success criterion.** |
| T-021 never modify game files | **SHIPPED** | `cs2kit verify baseline/check`, `cs2kit launch` refuses on mismatch (exit 4) |
| T-022 competitive sign-off | **TOOLED** | `docs/project/validation-log.md` sign-off template, every slot named to the task that fills it |
| T-023 specify v0.1 | **SHIPPED** | `docs/cli-reference.md` |
...
| T-029 documentation | **SHIPPED** | `docs/install.md`, `docs/troubleshooting.md`, `docs/compatibility-matrix.md` |
| T-030 update watch + drill | **SHIPPED** | `cs2kit watch check/record/drill`, appends a matrix row |
| T-031 Rosetta-27 exit plan | **SHIPPED** | `docs/project/rosetta-watch.md` (first dated entry, decommission notice pre-written), doctor grades the horizon |
| T-032 macOS beta testing | **TOOLED** | `docs/project/maintenance.md`, matrix row shape |
| T-033 community intake | **TOOLED** | `cs2kit report` bundles, intake procedure in `docs/project/maintenance.md` |
| T-034 quarterly upstream tracking | **TOOLED** | `docs/project/maintenance.md` review table + the MoltenVK trigger that re-opens T-012 |
...
* **T-010** and **T-020** are the two gates. Until T-020 is filled in, this project claims *nothing* about
  competitive play, and `docs/project/validation-log.md` says so in the same words.
* The one thing no amount of work here can change: **Rosetta 2's macOS 27 horizon** (`docs/project/rosetta-watch.md`).
...
* **Method:** `cl_showfps` read off the window by OCR (`scripts/fps_probe.py`), six readings 25 s apart,
  ...
  (`docs/benchmarking.md`). Settings: CS2's auto **Low** preset at the Retina backing
...
**The benchmark map is the whole point of the protocol.** `docs/benchmarking.md` specifies the
...
`docs/benchmarking.md`. It is the difference between measuring the game and measuring the observer.
